...
\begin{rmq}
Note that, for any modality $\modal$, we can define an equivalent
...
(this can be seen as a kind of HIT, but with a h-proposition
constraint instead of constructors for path type ; however, there are
no evidence that we are allowed to use these). Then, it is
straightforward\footnote{Maybe not ``straightforward'', but at least
  checked in Coq at
  \url{https://github.com/KevinQuirin/translation-mod/blob/master/theories/StrictMod.v}}
 to see that $\modal^S$ is a strict modality,
equivalent to $\modal$
\end{rmq}
