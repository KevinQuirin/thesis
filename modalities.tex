...
several equivalent characterizations of left-exactness.

\begin{prop}\label{prop:lex}
  Let $\modal$ be a modality. Then $\modal$ is left-exact if one of
  ...
  \item $\modal$ preserves pullback, in the sense that if
    ...
    \]
    is too.
  \item $\modal$ preserves path spaces, \ie{}
    \[
      \prodD A \Type {\prodD {x,y} A {\IsEquiv (\modal (\ap{\eta_A}))}}
    \]
    where $\modal (\ap{\eta_A}) : \modal(x = y) \to \eta_A x = \eta_A
    y$.
  \item If the modality is accessible\todo[fancyline]{Define accessibility}, the universe of modal types is modal, \ie{}
    \[
      \modal \left(\Type^\modal\right) \simeq \Type^\modal.
    \]
  \end{itemize}
\end{prop}
\begin{proof}
  \todo[inline]{Lex : Do the proof}
  \begin{itemize}
  \item This is the definition of left-exactness.
  \item 
  \item Let $A$ be a type, and $x,y:A$.
  \end{itemize}
\end{proof}
...
library. For example, the reflector $\modal$ of a modality is define,
in~\cite{hottlib} as
