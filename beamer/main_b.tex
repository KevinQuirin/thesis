...
\begin{frame}{Forcing in set theory}
  ...
  \note<4->[item]{The theorem of forcing states that there is a model
    MG with a set G in the power set of P, such that G is the actual
    set of approximations of the witness
  ...
  \begin{tikzpicture}
    ...
    \onslide<4->{
      ...
      \draw (2.5,3) node[right] {\phantom{witness} = $\lim G$};}
  \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \]
  \vfill
  \small{{\em Floris Van Doorn: Constructing the Propositional
      Truncation using Non-recursive HITs

      \smallskip

      S. Boulier, K. Quirin, E. Rijke, N. Tabareau: Colimits in HoTT
